...
fn simulate(prog: &[Operation], regs: (usize, usize, usize)) -> Vec<usize> {
    use Combo::*;
    use Operation::*;
    let mut ip = 0; // default instruction pointer
    let mut res = vec![];
    let (mut reg_a, mut reg_b, mut reg_c) = regs;
    while ip != prog.len() {
...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let prog: Vec<Operation> = prog_str
        ...
        .collect();
    let orig_prog: Vec<usize> = prog_str
        .trim_end()
        .split_once(": ")
        .unwrap()
        .1
