...
fn get_real_coord(empty: &[usize], coord: usize) -> usize {
    empty.iter().map(|empty_row_or_col| {
...
fn find_galaxy_locs(grid: &[&[char]], empty_cols: &[usize], empty_rows: &[usize]) -> Vec<(usize, usize)> {
    let mut res = Vec::new();
...
pub fn process(
    ...
    let sum = galaxy_locs.iter().combinations(2).map(|combination| {
